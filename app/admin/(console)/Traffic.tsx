...
function Tile({ k, v, s, tone }: { k: string; v: string; s: string; tone?: "good" }) {
  return (
    <div className="rounded-2xl bg-white/[0.04] p-4 ring-1 ring-white/10">
      <div className="text-[10.5px] font-bold uppercase tracking-[0.09em] text-white/45">{k}</div>
      <div className={`mt-1.5 text-[26px] font-extrabold leading-none tabular-nums ${tone === "good" ? "text-[#4ade9f]" : ""}`}>
        {v}
      </div>
      <div className="mt-1 text-[12px] text-white/45">{s}</div>
    </div>
  );
}
...
function Breakdown({
  ...
  return (
    <div className="rounded-2xl bg-white/[0.04] p-4 ring-1 ring-white/10">
      <h3 className="text-[11px] font-bold uppercase tracking-[0.09em] text-white/45">{title}</h3>
      {rows.length === 0 ? (
        <p className="mt-3 text-[13px] text-white/40">{empty}</p>
      ) : (
            ...
            <li key={r.label}>
              <div className="flex items-baseline justify-between gap-3 text-[13px]">
                ...
                <span className="shrink-0 tabular-nums text-white/55">
                  {r.visits}
                  {r.signups > 0 && (
                    <span className="ml-2 font-bold text-[#4ade9f]">
              ...
              <span className="mt-1.5 block h-[5px] overflow-hidden rounded-full bg-white/10">
                <span
                  className="block h-full rounded-full bg-white/35"
                  style={{ width: `${top > 0 ? Math.max(3, (r.visits / top) * 100) : 0}%` }}
                >
                  <span
                    className="block h-full rounded-full bg-[#4ade9f]"
  ...
  return (
    <section className="mt-10">
      <div className="flex items-baseline justify-between gap-3">
        ...
        <span className="text-[12px] text-white/40">{data.days} derniers jours</span>
      </div>
      ...
      {daily.length > 0 && (
        <div className="mt-3 rounded-2xl bg-white/[0.04] p-4 ring-1 ring-white/10">
          <h3 className="text-[11px] font-bold uppercase tracking-[0.09em] text-white/45">
            14 jours
          </h3>
          <div className="mt-3 flex h-[70px] items-end gap-[3px]">
            {daily.map((d) => (
              <span
                ...
                className="flex-1 rounded-t-[3px] bg-white/20"
                style={{ height: `${Math.max(2, (d.visits / peak) * 100)}%` }}
              >
                <span
                  className="block w-full rounded-t-[3px] bg-[#4ade9f]"
