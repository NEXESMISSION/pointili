...
export default async function AdminPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-royal">
          Plateforme
        </p>
        <h1 className="mt-0.5 text-[26px] font-extrabold text-charcoal">
          Tous les cafés
        </h1>
      </div>
      ...
      {(stats.expired > 0 || stats.expiring7d > 0 || stats.suspended > 0) && (
        <section className="rounded-2xl border border-[#f2d98a] bg-[#fff8e6] px-4 py-3">
          <p className="text-[11px] font-bold uppercase tracking-[0.1em] text-[#8a6d00]">
            À traiter
          </p>
      ...
      <section className="grid grid-cols-3 gap-2.5">
        ...
        <Stat label="Tours joués" value={stats.plays} />
      </section>

      <section>
        <p className="mb-2 text-[11px] font-bold uppercase tracking-[0.1em] text-slate">
          Cafés ({cafes.length})
        </p>
        {cafes.length === 0 ? (
          <p className="rounded-2xl border border-hair bg-white px-4 py-8 text-center text-[13px] text-slate">
      ...
      <section className="rounded-2xl border border-hair bg-white p-4 shadow-[0_10px_30px_-24px_rgba(40,18,59,.5)]">
        <p className="mb-2 text-[11px] font-bold uppercase tracking-[0.1em] text-slate">
          Message à tous
        </p>
        <BroadcastForm />
        ...
        <section>
          ...
          <ul className="space-y-2">
            {notices.map((n) => (
              <li
                key={n.id}
                className="flex items-start justify-between gap-3 rounded-2xl border border-hair bg-white px-4 py-3"
              >
                ...
                <form action={dismissNoticeAction.bind(null, n.id)} className="shrink-0">
                  <button
                    type="submit"
                    className="rounded-lg border border-hair px-2.5 py-1.5 text-[11px] font-bold text-slate active:scale-95"
                  >
                    Retirer
                  </button>
                </form>
          ...
          <ul className="divide-y divide-hair rounded-2xl border border-hair bg-white px-4">
            {actions.map((a, i) => (
              <li key={i} className="flex items-baseline justify-between gap-2 py-2.5">
                <span className="min-w-0">
                  <span className="block text-[12px] font-bold text-charcoal">
                ...
                <span className="shrink-0 text-[10.5px] text-slate">
    ...
    tone === "ok"
      ...
      : tone === "bad"
        ...
        : tone === "royal"
          ...
          : "text-charcoal";
  return (
    <div className="rounded-2xl border border-hair bg-white px-3 py-2.5">
      <p className="text-[9px] font-bold uppercase tracking-[0.08em] text-slate">{label}</p>
